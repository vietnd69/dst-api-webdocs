...
## Overview
`catcoonden` is the constructor for the Catcoon Den prefab—a unique structure that acts as a boss encounter with multiple lives, inventory storage for catcoons, and dynamic spawning behavior. It integrates with multiple components: `workable` (for hammering), `childspawner` (for spawning and regenerating catcoons), `inventory` (to hold items stored by catco